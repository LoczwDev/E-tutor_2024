...
export const CheckOutFormCart = ({ data, clientSecret }) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: ({ amount, courseIds, payment_info, emailOrder }) =>
      createOrder({ amount, courseIds, payment_info, emailOrder }),
    onSuccess: (data) => {
      setDataOrder(data);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!emailPattern.test(emailOrder)) {
      toast.error("Vui lòng nhập 1 email hợp lệ");
      return;
    }
    if (selectedMethod === "new_card") {
      ...
      try {
        ...
        if (error) {
          ...
          mutate({
            courseIds: data.map((item) => item._id),
      ...
      >
        <div className="w-[55%]">
          ...
          <div className="w-full mb-5">
            <label htmlFor="emailOrder" className={`${styled.label}`}>
          ...
          <div className="w-full flex flex-col gap-3 border-b pb-5">
            {dataCourseCheckOut?.map((item, index) => (
              <div key={index} className="flex items-start gap-3">
                ...
                <div className="flex w-full flex-col h-24 justify-between">
